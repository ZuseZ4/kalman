...
//   std::normal_distribution<T> noise(0, 1);
  ...
  for (size_t i = 1; i <= N; i++) {
    ...
    u.dtheta() = 1.0;//std::sin(T(2) * T(M_PI) / T(N)) * (1 - 2 * (i > 50));

    // Simulate system
    x = sys.f(x, u);

    // Add noise: Our robot move is affected by noise (due to actuator failures)
    x.x() += systemNoise * 0.5;//noise(generator);
    x.y() += systemNoise * 0.5;//noise(generator);
    x.theta() += systemNoise * 0.5;//noise(generator);
    ...
    // Orientation measurement
    {
      ...
      // orientation.theta() += orientationNoise * 0.5;//noise(generator);
    ...
    //   // Measurement is affected by noise as well
    //   position.d1() += distanceNoise * 0.5;//noise(generator);
    //   position.d2() += distanceNoise * 0.5;//noise(generator);
